...
export default function SendPage() {
  ...
  // Stable across retries so a double submit cannot create two transfers.
  ...
  const requestQuote = useCallback(async () => {
    ...
    try {
      ...
      if (!best) {
        setRampBlocked('Pollar returned no Bolivian off-ramp quote for this amount.');
        setQuote(indicative);
        setStep('recipient');
        return;
      }
  ...
  }, [quote, idempotencyKey, recipientName, fields, rampQuote, router]);
  ...
  if (!isAuthenticated || !sessionReady) {
    return (
      <main className="mx-auto max-w-xl px-4 py-10">
        ...
        <SignInPanel onSignedIn={() => setSessionReady(true)} />
      </main>
    );
  }
